...
from agents.model import MLP, Unet
...
class Consistency_QL(object):
    def __init__(self,
        ...
        # self.model = Unet(n_channel, D=256, device=device)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None):
        ...
        for itr in range(iterations):
            ...
            N = math.ceil(math.sqrt((itr * (150**2 - 4) / iterations) + 4) - 1) + 1
            boundaries = kerras_boundaries(7.0, 0.002, N, 80.0).to(self.device)

            z = torch.randn_like(action)
            t = torch.randint(0, N - 1, (action.shape[0], 1), device=self.device)
            t_0 = boundaries[t]
            t_1 = boundaries[t + 1]
            bc_loss = self.actor.loss(state, action, z, t_0, t_1, self.ema_actor)
            
            if loss_ema is None:
                loss_ema = bc_loss.item()
            else:
                loss_ema = 0.9 * loss_ema + 0.1 * bc_loss.item()
